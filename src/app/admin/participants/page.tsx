...
export default function AdminParticipantsPage() {
  ...
  const load = useCallback(async () => {
    ...
    const res = await getJson<{ participants: Participant[] }>(`/api/admin/participants${qs ? `?${qs}` : ""}`);
    if (!res.ok || !res.data) {
  ...
  return (
    ...
    >
          ...
          {STATUS_TABS.map((t, i) => (
            <button
              ...
              style={{
                ...
                borderRight: i < STATUS_TABS.length - 1 ? "1px solid var(--c-line)" : "none",
      ...
      {rows && (
        <div style={{ background: "#fff", border: "1px solid var(--c-line)", borderRadius: "var(--r-lg)", overflow: "hidden" }}>
          <div
            style={{
              display: "grid",
          ...
          {rows.map((r) => {
            ...
            return (
              ...
              >
                ...
                <div style={{ fontSize: 12, color: "var(--c-ink-soft)" }}>{r.lga}</div>
                <div style={{ fontSize: 12, fontFamily: "var(--font-mono)", color: "var(--c-gold-deep)" }}>{r.cohort}</div>
                <div style={{ fontSize: 12 }}>{r.skill}</div>
                <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                  <div style={{ flex: 1, height: 4, background: "var(--c-cream-deep)", borderRadius: 2, overflow: "hidden" }}>
                    <div style={{ height: "100%", width: `${r.progressPercent}%`, background: "linear-gradient(90deg, var(--c-magenta), var(--c-gold))" }} />
                  </div>
                  <div style={{ fontSize: 11, color: "var(--c-gold-deep)", fontFamily: "var(--font-mono)", minWidth: 30 }}>{r.progressPercent}%</div>
                </div>
                <div style={{ fontFamily: "var(--font-display)", fontWeight: 800, fontSize: 16, color: "var(--c-gold-deep)" }}>{r.medals}</div>
                <div style={{ display: "flex", gap: 4, alignItems: "center", fontSize: 12 }}>
                  <FlameIcon size={12} style={{ color: "var(--c-magenta)" }} />
                  <span style={{ fontFamily: "var(--font-display)", fontWeight: 700 }}>{r.streak}d</span>
                </div>
                <div>
                  <span
                    style={{
          ...
          {rows.length === 0 && (
            <div style={{ padding: 40, textAlign: "center", color: "var(--c-ink-soft)", fontSize: 13 }}>No participants match these filters.</div>
          )}
        </div>
      )}
    </AdminShell>
