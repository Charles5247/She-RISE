...
export async function GET(req: Request) {
  ...
  const participants = rows.map((r) => {
    const totalLessons = (
      db.prepare(`SELECT COUNT(*) as c FROM lessons l JOIN pathways p ON p.id = l.pathway_id WHERE p.skill_category = ?`).get(r.skill_category) as { c: number }
    ).c;
    const doneLessons = (
      db.prepare(`SELECT COUNT(*) as c FROM lesson_progress WHERE user_id = ? AND status = 'done'`).get(r.id) as { c: number }
    ).c;
    const medalsCount = (db.prepare(`SELECT COUNT(*) as c FROM medals WHERE user_id = ?`).get(r.id) as { c: number }).c;
